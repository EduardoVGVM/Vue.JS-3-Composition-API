...
const items = ref([
  ...
  { id: 4, label: "Another item", purchased: false, highPriority: true }
])
...
<template>
  <div class="header">
    <h1>{{ header }}</h1>
    <button v-if="editing" class="btn" @click="doEditing(false)">Cancel</button>
  ...
  <br />
  <ul>
    <li v-for="(item, index) in reversedItems" @click="togglePurchased(item)" :key="item.id" class="static-class"
      :class="{ strikeout: item.purchased, priority: item.highPriority }">{{ item.label }}</li>
  </ul>
  <p v-if="!items.length">
    Nothing here...
  </p>
